...
public partial class HwpxHandler
{
    ...
    public string ViewAsText(int? startLine = null, int? endLine = null,
                              ...
                              string? range = null)
    {
        // `range` is the xlsx-only cell-range subset. HWPX is a flow format
        // with no cell addressing, so reject it the same way docx/pptx do
        // rather than silently ignoring a selector the user asked for.
        Core.ViewRangeGuard.RejectTextRange(range, "hwpx");
