        ...
        cmd.SetAction(result => SafeRun(() =>
        {
            ...
            try
            {
                ...
                if (!proc.WaitForExit(60_000))
                {
                    ...
                    const string timeoutMessage = "LibreOffice conversion timed out after 60 seconds.";
                    if (json) Console.WriteLine(OutputFormatter.WrapEnvelopeError(timeoutMessage));
                    else Console.Error.WriteLine($"Error: {timeoutMessage}");
                    return 1;
                }
                ...
                if (proc.ExitCode != 0)
                {
                    ...
                    if (json) Console.WriteLine(OutputFormatter.WrapEnvelopeError(
                        $"LibreOffice exited with code {proc.ExitCode}: {stderr}"));
                ...
                if (converted == null)
                {
                    if (json) Console.WriteLine(OutputFormatter.WrapEnvelopeError("LibreOffice produced no output"));
                ...
                if (json)
                    Console.WriteLine(OutputFormatter.WrapEnvelope(new JsonObject
                    {
